...
class User < ActiveRecord::Base
  ...
  after_save do
    if saved_change_to_uploaded_avatar_id?
      UploadReference.where(target: self).where.not(upload_id: self.uploaded_avatar_id).destroy_all
      UploadReference.find_or_create_by!(upload_id: self.uploaded_avatar_id, target: self) if self.uploaded_avatar_id.present?
    end
  end

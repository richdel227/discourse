...
class Badge < ActiveRecord::Base
  ...
  after_save do
    if saved_change_to_image_upload_id?
      UploadReference.where(target: self).where.not(upload_id: self.image_upload_id).destroy_all
      UploadReference.find_or_create_by!(upload_id: self.image_upload_id, target: self) if self.image_upload_id.present?
    end
  end
